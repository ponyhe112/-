...
$sql = "select admin_id from admin where admin_id={$username}";
...
<h3 style="text-align: center"><?php echo $username; ?>号管理员，您好</h3><br/>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $passb = $_POST["pass2"];
    if ($passa == $passb) {
        $sql = "update admin set password='{$passa}' where admin_id={$username}";
        $res = mysqli_query($dbc, $sql);
        if ($res == 1) {
